...
const MarkdownTextImpl: FC = () => {
  return (
    <MarkdownTextPrimitive
      ...
      preprocess={(text) => text.split("__JSON_DATA__")[0]}
    />
  );
};
...
const defaultComponents = memoizeMarkdownComponents({
  ...
  p: ({ className, children, ...props }) => {
    // Get sources data from the message
    ...
    let sourcesData: any[] = [];
    if (text && text.includes("__JSON_DATA__")) {
      try {
        const jsonPart = text.split("__JSON_DATA__")[1];
    ...
    // Helper function to process text and extract citations
    const processCitations = (text: string): (string | React.ReactNode)[] => {
      // Match any [Source N] pattern, regardless of what follows
      ...
      while ((match = citationRegex.exec(text)) !== null) {
        ...
        // Get actual source data from the JSON
        const sourceData = sourcesData[sourceIndex];
        
        if (match.index > lastIndex) {
          parts.push(text.substring(lastIndex, match.index));
        }

        // Extract snippet from the surrounding text (optional enhancement)
        const snippetMatch = text.substring(match.index).match(/\[Source\s+\d+\]\s*\(([^)]+)\)/i);
        const snippet = snippetMatch ? snippetMatch[1] : (sourceData?.title || 'No preview available');

        parts.push(
          <CitationMarker
            key={`citation-${sourceNum}-${match.index}`}
            id={`Source ${sourceNum}`}
            type={sourceData?.type || 'unknown'}
            snippet={snippet}
            fullText={sourceData?.text}
            url={sourceData?.url}
          />
        );
    ...
    const processChildren = (child: any): any => {
      ...
      if (React.isValidElement(child)) {
        ...
        if (props.children) {
          return React.cloneElement(child, {}, processChildren(props.children));
        }
      }
